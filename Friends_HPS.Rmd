...
author: "David Garcia, Hannah Metzler, Janna Lasser, Max Pellert, Anna Di Natale <br> *Graz University of Technology*"
...
Figure \@ref(fig:episodes) shows the OMG per episode in each season of the series, There is a tendency to more OMG over the lifetime of the series, from less than 3 OMG per episode in the first season to more than 6 in the last one. To compare Friends to contemporary TV shows and movies we applied the same analysis to the 2020 edition of the Corpus Of Contemporary American English (COCA) [@davies2010corpus]. In our [github repository]() we share the final yearly counts of our analyses, as we are not allowed to share the raw text of the corpus by the terms to access it.
...
omgR <- sum(genderdf$femaleCount)/(sum(genderdf$femaleCount+genderdf$maleCount))
...
totR<-sum(genderdf$femaleTotal)/(sum(genderdf$femaleTotal+genderdf$maleTotal))
...
```{r twitterLA, fig.width=7, fig.height=7, cache=TRUE, message=F, fig.cap="Heatmap of OMG tweets in the Los Angeles area.", echo=F, fig.align='center'}
...
knitr::kable(head(omgstates %>% select(sate=full, perMillion) %>% arrange(desc(perMillion)),10))
```
...
knitr::kable(tail(omgstates %>% select(sate=full, perMillion) %>% arrange(desc(perMillion)),10))
```
...
We generated 10.000 random texts with GPT-2 to have an idea of the contexts that can follow "Oh my God!". Below we show the first five as examples:
